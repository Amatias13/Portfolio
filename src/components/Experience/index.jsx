...
function TimelineItem({ item, index, lang }) {
  return (
    <motion.div className="timeline-item" initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }}>
      ...
      <ul className="timeline-item__bullets">
        {item.bullets[lang].map((b) => (
          <li key={b.slice(0, 32)} className="timeline-item__bullet">
            {b}
          </li>
        ))}
      </ul>
    ...
    <Section id="experiencia">
      ...
      {education.length > 0 && (
        <div className="education">
          ...
          {education.map((edu, i) => (
            <motion.div key={edu.id} className="edu-card" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }}>
              <div className="edu-card__icon">🎓</div>
              <div>
                <div className="edu-card__degree">{edu.degree[lang]}</div>
                <div className="edu-card__school">
                  {edu.school} · {edu.location}
                </div>
                <div className="edu-card__period">{edu.period}</div>
                {edu.description && <p className="edu-card__description">{edu.description[lang]}</p>}
              </div>
            </motion.div>
          ))}
        </div>
