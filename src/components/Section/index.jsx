...
export default function Section({ id, children }) {
  ...
  return (
    <motion.section id={id} ref={ref} className="section" initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 24 }} transition={{ duration: 0.65, ease: [0.22, 1, 0.36, 1] }}>
      {children}
    </motion.section>
  );
}
